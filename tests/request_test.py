...
def main():
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in ('proxy', 'node'):
        exit_error('usage: driver_test.py <proxy|node>')
    ...
    resp = requests.post(base_url + '/wd/hub/session/' + session_id + '/elements', json={"value": "//a", "using": "xpath", "sessionId": session_id})
    if resp.status_code != 200:
        exit_error('failed to execute xpath')

    elements = json.loads(resp.content.decode())['value']
    elem_dict = elements[1]
    elem_key = list(elem_dict.keys())[0]
    elem_id = list(elem_dict.values())[0]

    req_json = get_attribute_request_json(session_id, elem_key, elem_id)
    resp = requests.post(base_url + '/wd/hub/session/' + session_id + '/execute/sync', json=req_json)

    outer_html = json.loads(resp.content.decode())['value']
