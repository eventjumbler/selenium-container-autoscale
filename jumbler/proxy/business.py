...
class BusinessLogic(object):
    ...
    async def start_node(self, browser, os_system):
        _LOG.info('Start Browser: %s - OS: %s', browser, str(os_system))
        ...
        if status in (Status.NOT_AVAILABLE, Status.RUNNING):
            ...
            result["container_id"] = self.selenium.start_new_node(self.proxy_container_id, browser, result.get("selenium_node_id"))
