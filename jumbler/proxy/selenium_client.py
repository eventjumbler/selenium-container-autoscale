...
class SeleniumClient(object):
    ...
    async def _get_active_sessions(self, container):
        try:
            status, resp_json = await rest_client.http_get(
        ...
        if status != 200:
            logger.error('GET /wd/hub/sessions status: %s  %s', status, resp_json)
            return False, status, []
    ...
    async def _launch_driver_on_container(self, req_body, container_name):
        ...
        try:
            status_code, resp_json = await rest_client.http_post(
                url, data=req_body,
                session=req_session
            )
        except:
